...
export class RoverAvecCarte {

    constructor(private carte, private position: Position, private direction: Direction) {
    }

    executeCommands(movements: string): string {
        for (const movement of movements) {
            const positionSaved = this.position;
            this.moveV3(Movement[movement]);
            if(this.position.y > this.carte.height || this.position.y < 0) {
                this.position = positionSaved;
            }
        }
